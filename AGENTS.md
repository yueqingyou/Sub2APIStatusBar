...
- 页面标题下方的说明只保留会影响用户判断或操作的状态、风险和恢复提示；“外观、连接、硬件屏与更新”等仅枚举页面内容的副标题，以及与控件标签重复的注解，必须删除或收敛为更短的就地提示。硬件屏界面遵循同一原则，不常驻显示页码、按键教程或重复链路状态。
- 全 App 的卡片、输入框、分段控件和页签默认通过背景色阶、间距与选中填充表达层级，不叠加无语义描边；移除框线时必须同时核对浅色和深色外观的边界辨识度。图表网格、进度轨道、时间线等承载数据或结构语义的线条可以保留。
- 当前页签已经明确页面名称时，内容区不得再次显示同名大标题；可见字段标签与占位文字不得逐字重复，账号名与邮箱相同时也只显示一次。动态时间、错误、风险、恢复提示和会改变操作判断的状态不属于冗余注解。
- 未登录界面必须复用登录后界面的公共品牌标识、背景、卡片层级和控件样式，不得保留平行的旧版首屏。未登录时菜单栏只显示模板化品牌图标，并通过 tooltip 和辅助功能标签提示未登录；不得继续渲染已配置的业务栏目或旧快照。
- 菜单栏弹窗打开时不得自动聚焦账号、密码或令牌输入框；这些凭据输入必须关闭 macOS 自动更正和文本建议，避免候选浮层遮挡输入内容。刷新间隔、手动令牌等非主要登录项应收纳为默认折叠的高级选项。
...
- Mac 端硬件监控功能必须默认关闭，并且只能在用户启用后创建 Core Bluetooth 管理器、请求系统权限和开始扫描；没有硬件的用户不得承担后台扫描或蓝牙授权提示。
- BLE 协议版本、UUID 和二进制字段必须在 `Sources/Sub2APIStatusCore/HardwareMonitorBLEProtocol.swift` 与 `硬件开发/ESP32-S3-RLCD-4.2/firmware/main/ble_link.*` 间同步更新并测试。连接或版本握手只表示传输链路可用，不得冒充页面数据已经同步。
- 已绑定设备的 GATT 服务或特征发生变化时，固件必须提升持久化服务表版本并通过标准 Service Changed 指示使既有 Mac 缓存失效；macOS 客户端必须处理 `didModifyServices` 并重新发现服务。不得把删除绑定、重新配对或再次 USB 初始化作为正常升级路径。
- 硬件屏不得接收或保存 TokenRouter 密码、访问令牌、刷新令牌、Codex 节点密钥或原始 API 响应。在发送真实监控数据前，必须实现由实体长按开启的有限配对窗口和持久化绑定；未绑定链路只允许传输不含业务数据的兼容性握手。
- 硬件监控屏允许使用多节 3500 mAh 18650，但多节电芯必须采用与串并联结构匹配的 BMS、均衡、充电、电源转换和反向供电隔离；不得把多节裸电芯直接接入板载单节 18650 路径。
- 硬件屏每个页面的完整数据同步周期必须由用户在 Mac 设置中分别配置，并由当前显示页面决定刷新计划；页面切换时只刷新已经过期的新页面，不得让较短周期页面覆盖七天额度等低频页面。
- 板载电量必须由固件真实采样，并在四个普通页面的同一固定位置显示；电量采样周期由用户在 Mac 设置中独立配置并通过监控协议下发，固件必须限制为低频范围、一次性释放 ADC，且显示值未变化时不得刷新电子纸。
- 硬件屏定位为低频快照，不得显示或传输实时占用并发、活动任务、运行数或等待数等会因同步延迟产生误导的业务指标；Mac、网络、TokenRouter、BLE 和数据状态等用于解释数据停滞的设备健康信息可以保留。
- 页面数据刷新与离线检查必须使用独立计划。轻量离线检查不得更新完整页面指标；用户关闭独立检查时，设置界面必须明确提示离线发现延迟将退化为当前页面刷新周期。
- 固件升级协议必须与监控数据协议分离并保持向后兼容，使新版 Mac App 即使无法解析旧版监控页面协议，仍能通过已绑定的加密 BLE 链路升级具备 OTA 能力的旧固件。升级只允许使用 App 内置、经清单大小和 SHA-256 校验且版本不低于设备的固件；设备还必须校验项目名、目标版本、完整镜像哈希和 ESP 镜像结构。
- BLE 状态通知的前 16 字节是跨监控协议版本的升级兼容前缀，包含固定页编号、固件版本和升级能力、状态、错误及接收字节数；后续监控协议只能在其后追加字段，不得改写这些偏移或要求监控握手成功后才读取升级状态。
- 首次启用 BLE 固件升级时允许通过 USB 一次性写入 bootloader、双 OTA 分区表、factory 应用和 OTA 元数据，但不得全片擦除或覆盖保存蓝牙绑定的 NVS。此后正常升级必须由更新后的 macOS App 通过 BLE 完成，并在新固件重启回连后核对目标版本；启动失败时必须由 ESP-IDF 回滚到上一可用分区。
- macOS App 的发布包必须内置同一源码构建出的 ESP32 固件和版本化清单；App 构建、自动更新安装和 Release 校验都必须拒绝缺失、大小不符或 SHA-256 不匹配的内置固件，避免用户先升级 App 后才发现硬件固件不可用。
- ESP32-S3-RLCD-4.2 的固件、公开硬件文档和验收记录统一保存在 `硬件开发/ESP32-S3-RLCD-4.2/`；设备专属 Flash 备份和临时产物只放在该子项目的 `tmp/yyyy.mm.dd/` 下且不得提交。

## 本机验证约束

- 本机新增或管理系统依赖时统一使用 MacPorts，不得使用 Homebrew 或执行 `brew` 命令；上游只提供 Homebrew 示例时，必须转换为对应的 MacPorts 包并核对实际安装路径。
...
- 最终 App 构建必须传播 `swift build` 的非零退出并确认本轮二进制确实生成，不得在编译失败后复用旧产物；本机 Intel Swift 5.7 的 Release WMO 默认使用单前端线程，避免多线程优化器随机崩溃。
- `.app` 编译成功后必须核对 bundle 元数据、可执行文件架构和签名状态，并在交付中提供本轮新生成 App bundle 的绝对路径供用户本地测试；不得只提供 SwiftPM 裸可执行文件路径，也不得覆盖或替换正式安装版本。
- 临时运行候选 App 时不得只设置 `HOME` 并假定 Foundation 用户目录已经隔离；`FileManager.homeDirectoryForCurrentUser` 仍可能指向真实用户目录，启用“登录时打开”的测试配置会把真实 LaunchAgent 改写为临时 App 路径。验证前必须使用代码支持的显式目录注入，或关闭临时配置的登录启动并在运行前后核对、恢复真实 LaunchAgent、配置和凭据文件；不得把未验证的环境变量隔离表述为已经成立。
