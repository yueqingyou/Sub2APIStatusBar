...
- 用户界面不得使用 Unicode 表情、字符画或文本符号冒充图标；优先使用与最低支持 macOS 兼容的 SF Symbols，动态符号名必须提供 SF Symbol 兜底。
- 状态标签统一使用小圆角连续矩形，不使用与卡片圆角不协调的全胶囊；只有进度条等确有形状语义的控件可以使用胶囊形状。
- 外观设置切换到跟随系统、浅色或深色时必须立即同步 SwiftUI 色彩环境、AppKit popover、承载视图和当前窗口，不得要求关闭弹窗或重启 App 后才生效。
- 跟随系统模式必须先解析为当前唯一的有效浅色或深色状态，再统一应用于 SwiftUI 与 AppKit；不得让材质、语义色和自定义色各自继承不同层级的外观。
- 设置页等长页面不得直接观察整份高频刷新模型；只订阅界面实际使用且去重后的状态，并优先使用轻量 SwiftUI 控件，避免后台快照刷新或大量 AppKit 控件桥接放大切页卡顿。
