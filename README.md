...
```bash
VERSION=v0.1.33 ./scripts/build-app.sh
```
...
```bash
VERSION=v0.1.33 ARCHITECTURE=universal ./scripts/build-app.sh
```
...
for ARCHITECTURE in x86_64 arm64 universal; do
  VERSION=v0.1.33 ARCHITECTURE="$ARCHITECTURE" ./scripts/package-release.sh
done
```
...
```text
dist/Sub2APIStatusBar-0.1.33-macOS-x86_64.zip
dist/Sub2APIStatusBar-0.1.33-macOS-arm64.zip
dist/Sub2APIStatusBar-0.1.33-macOS-universal.zip
```
...
Pushing a `v*` tag runs the GitHub Actions release pipeline on the explicit Intel runner. CI builds and verifies `x86_64`, `arm64`, and Universal 2 archives, then copies Universal 2 to an alphabetically earlier `compat-macOS.zip` name for legacy updater compatibility. The draft release is published only after CI verifies that the compatibility archive is the first ZIP returned by the API, that its digest matches Universal 2, and that all eight ZIP/checksum assets are present. Newer updaters still prefer the exact process architecture. Local `gh release create` is not part of the normal release path.
